...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // FIX: tu backend devuelve "mensaje", no "message"
      const msg = res.data?.mensaje || '¡Registro exitoso!';
      setSuccessMessage(msg);

      setTimeout(() => navigate('/login'), 1500);
    } catch (err) {
      // FIX: cambiar "message" por "mensaje"
      setErrorMessage(
          ...
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl p-8 border border-hotel-gold/20 hover:shadow-3xl transition-all duration-500 hover:border-hotel-gold/40 animate-slide-up">
            ...
            <div className="text-center mb-8 animate-fade-in-delay">
              ...
              <p className="text-gray-600 text-sm leading-relaxed">
                Únete al sistema de gestión del Hotel SGS
              </p>
            </div>
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              {errorMessage && (
                <div className="px-4 py-2 rounded-md bg-red-50 border border-red-100 text-red-700 text-sm">
                  {errorMessage}
                </div>
              )}
              {successMessage && (
                <div className="px-4 py-2 rounded-md bg-green-50 border border-green-100 text-green-700 text-sm">
                  {successMessage}
                </div>
              )}
              ...
              <div className="animate-fade-in-delay-3">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-hotel-gold focus:border-hotel-gold transition-all duration-300 bg-white/80 backdrop-blur-sm hover:bg-white/90 focus:scale-105"
                  placeholder="usuario@hotelsgs.com"
                />
              </div>
